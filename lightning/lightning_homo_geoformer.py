...
class PL_H_GeoFormer(pl.LightningModule):
    ...
    def save_checkpoint(self, filepath, **kwargs):
        ...
        super(PL_H_GeoFormer, self).save_checkpoint(filepath, **kwargs)
        ...
        if dataloader_idx == 1:
            ...
            if 'image0' not in batch:
                ...
                try:
                    ...
                    # with torch.no_grad:
                    # with self.profiler.profile("GeoFormer"):
                    #     ret = self.hpatches.cal_one(batch_idx=batch_idx, isprint=True, **batch)
                    # with torch.cuda.amp.autocast():  # 计算错误
                    #     ret = self.hpatches.cal_one(batch_idx=batch_idx, isprint=True, **batch)
                    ...
                    cache_func_handle(batch=batch, version='ours')
                except:
                    import traceback
                    traceback.print_exc()
    ...
    def on_validation_epoch_end(self):
        ...
        if self.trainer.is_global_zero or 1:
            ...
            if self.trainer.is_global_zero and total_val_samples >= 16:  # and self.global_step > 0:
                ...
                # print(hpatches_res['summary'])
                print('[r{}] hpatch auc (len={}): {}'.format(self.global_rank, total_val_samples, auc))
                # self.logger.experiment.add_scalar(
                #     'val_hpatches/auc@3px_on_epoch', auc[1],
                #     global_step=self.current_epoch)
                # self.logger.experiment.add_scalar(
                #     'val_hpatches/auc@5px_on_epoch', auc[2],
                #     global_step=self.current_epoch)
                # self.logger.experiment.add_scalar(
                #     'val_hpatches/auc@10px_on_epoch', auc[3],
                #     global_step=self.current_epoch)

        # self.trainer.strategy.barrier()  # 与 self.all_gather 联用
        # for metric, score in auc_dict.items():
        #     self.log(metric, score, sync_dist=True)
