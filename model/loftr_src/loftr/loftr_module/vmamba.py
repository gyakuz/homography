...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.out_proj = nn.Linear(hidden_dim*ssm_ratio, hidden_dim)
        ...
        self.dropout = nn.Dropout(ssm_drop_rate) if ssm_drop_rate > 0. else nn.Identity()
        self.eps = 1e-6   
    ...
    def forward(self, input1: torch.Tensor,input2: torch.Tensor, H, W):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input1,input2)
        else:
            return self._forward(input1,input2, H, W)
